...
.PHONY: secrets/%
secrets/%:
	@ENV="$(ENV)" TARGET="$(TARGET)" SECRET_NAME="$(SECRET_NAME)" SECRET_VALUE="$(SECRET_VALUE)" RENDER_API_KEY="$(RENDER_API_KEY)" RENDER_SERVICE_ID="$(RENDER_SERVICE_ID)" RENDER_QA_SERVICE_ID="$(RENDER_QA_SERVICE_ID)" RENDER_STAGING_SERVICE_ID="$(RENDER_STAGING_SERVICE_ID)" RENDER_PROD_SERVICE_ID="$(RENDER_PROD_SERVICE_ID)" RAILS_MASTER_KEY="$(RAILS_MASTER_KEY)" MYSQL_PASSWORD="$(MYSQL_PASSWORD)" POSTGRES_PASSWORD="$(POSTGRES_PASSWORD)" DATABASE_PASSWORD="$(DATABASE_PASSWORD)" GITHUB_REPOSITORY="$(GITHUB_REPOSITORY)" sh $(SECRETS_SCRIPT) "$*"
