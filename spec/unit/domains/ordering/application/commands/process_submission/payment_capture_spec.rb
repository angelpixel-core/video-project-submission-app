...
RSpec.describe Ordering::Application::Commands::ProcessSubmission do
  it "processes a submission and creates payment via the payment command" do
    ...
    payment_command = lambda do |project:, provider:, payment_method_type:, gateway:|
  ...
  it "resolves an alternate provider through the gateway container" do
    ...
    submission = Ordering::Application::DTO::Submission.from_project(project_bridge, fulfillment_account: pm, payment_provider: "stripe")
    ...
    result = described_class.call(
      ...
      payment_command: lambda { |project:, provider:, payment_method_type:, gateway:|
