    ...
    module Event
      class Ingest
        ...
        def call
          ...
          event, created = Repository.upsert_received_event(
            provider: provider,
            event_id: event_id,
          ...
          event = Repository.find_by_provider_and_event_id(provider: provider, provider_event_id: event_id)
        ...
        def payment_for_event(event_data)
          ...
          return Payments::Payment::Repository.find_by_id(payment_id) if payment_id.present?

          reference = event_data["data"].to_h["provider_reference"].presence || event_data["provider_reference"].presence
          return if reference.blank?

          Payments::Payment::Repository.find_by_provider_reference(reference)
        end
