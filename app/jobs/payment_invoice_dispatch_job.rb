class PaymentInvoiceDispatchJob < ApplicationJob
  retry_on ActiveRecord::Deadlocked, wait: 1.second, attempts: 5

  def perform(payment_invoice_delivery_intent_id)
    intent = PaymentInvoiceDeliveryIntent.find_by(id: payment_invoice_delivery_intent_id)
    ...
    intent.payment.with_lock do
      ...
      PaymentInvoiceMailer.invoice_ready(intent).deliver_now
